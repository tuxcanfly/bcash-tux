...
class Script {
    ...
    for (let ip = 0; ip < this.code.length; ip++) {
      ...
      switch (op.value) {
        ...
        case opcodes.OP_NOTIF: {
          ...
          if (!negate) {
            ...
            if ((flags & Script.flags.VERIFY_SIGHASH_FORKID)
              && (flags & Script.flags.VERIFY_MINIMALIF)) {
        ...
        case opcodes.OP_NUM2BIN: {
          ...
          if (size > consensus.MAX_SCRIPT_PUSH) {
            throw new ScriptError('PUSH_SIZE', op, ip);
          }
          ...
          const n = stack.get(-1);
          const v = ScriptNum.decode(n, true, size);
          ...
          let signbit = 0x00;
          if (raw.length > 0) {
            signbit = raw[raw.length - 1] & 0x80;
            raw[raw.length - 1] &= 0x7f;
          }

          while (raw.length < size - 1)
            raw = Buffer.concat([raw, Buffer.from('00', 'hex')]);

          raw = Buffer.concat([raw,
            Buffer.from(signbit.toString(16, 'hex'), 'hex')]);
        ...
        case opcodes.OP_BIN2NUM: {
          ...
          const n = stack.get(-1);
          const v = ScriptNum.fromRaw(n);

          stack.pop();
          stack.push(v.toRaw());

          // The resulting number must be a valid number.
          if (!ScriptNum.isMinimal(v.toRaw()) || v.length > 4)
  ...
  static verify(input, witness, output, tx, index, value, flags) {
    ...
    input.execute(stack, flags, tx, index, value);
    ...
    output.execute(stack, flags, tx, index, value);
  ...
  if (flags & Script.flags.VERIFY_SIGHASH_FORKID) {
    if (flags & Script.flags.VERIFY_COMPRESSED_PUBKEYTYPE) {
      if (!common.isCompressedEncoding(key))
        throw new ScriptError('COMPRESSED_PUBKEYTYPE');
    }
  }
