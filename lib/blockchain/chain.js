...
class Chain extends AsyncEmitter {
  ...
  async verifyInputs(block, prev, state) {
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      sigops += tx.getSigopsCount(view, state.flags);
  ...
  async getTarget(time, prev) {
    ...
    if (this.state.hasDAA()) {
      ...
      const target = this.computeTarget(first, last);
  ...
  /**
   * Compute a target based on the work between 2 blocks and the timem
  ...
  computeTarget(first, last) {
    ...
    // Compute (2^256 / W) -1
    return new BN(1).iushln(256).div(work).isubn(1);
  }
